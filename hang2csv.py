    ...
    if "img" in event_type or "loc" in event_type:
        for attachment in event["chat_message"]["message_content"]["attachment"]:
            ...
            if "PLUS_PHOTO" in item["type"]:
                name = urllib.unquote(item["plus_photo"]["url"].split("/")[-1].decode("ascii"))
                img = urllib.unquote(name)
...
print "Found {} conversations".format(len(data))
convNum = 1
...
for conversation in data:
    ...
    with open(fname, "w") as outfile:
        writer = CsvUTF8Writer(outfile, delimiter=',', quotechar='"', quoting=csv.QUOTE_MINIMAL)
